...
//-------------------------------------------------------------------------------------------------
template <typename T>
inline bool FCreateHook(T *const pTarget, T *const pDetour)
    ...
    if (fdwReason == DLL_PROCESS_ATTACH)
    {
        ...
        if (dwTemp >= 6 && dwTemp < MAX_PATH)
        {
            ...
            if (pDelim >= g_wBuf+4 && pDelim <= g_wBuf+MAX_PATH-g_dwPathMargin)
            {
                ...
                {
                    size_t szOffset = reinterpret_cast<size_t>(SHGetSpecialFolderPathWStub) - (reinterpret_cast<size_t>(pAddress) + dwPatchSize);
                    ...
                    if (VirtualProtect(pAddress, dwPatchSize, dwOldProtect, &dwTemp))
                    {
                        ...
                        if (VirtualProtect(pAddress, dwPatchSize, PAGE_EXECUTE_READWRITE, &dwOldProtect))
                        {
                            szOffset = reinterpret_cast<size_t>(RegCreateKeyExWStub) - (reinterpret_cast<size_t>(pAddress) + dwPatchSize);
